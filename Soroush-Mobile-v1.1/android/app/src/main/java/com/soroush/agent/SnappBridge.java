...
public final class SnappBridge {
    ...
    public static void status(String s){Listener l=listener;if(l!=null)l.onStatus(s);}
    public static boolean openSnapp(Context c){String[] pkgs={"cab.snapp.passenger","cab.snapp.passenger.play"};for(String p:pkgs){Intent i=c.getPackageManager().getLaunchIntentForPackage(p);if(i!=null){i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);c.startActivity(i);return true;}}status("اپ Snapp روی این گوشی پیدا نشد.");return false;}
    public static boolean launch(Context c,AgentCommand cmd){pending.set(cmd);if(!openSnapp(c)){pending.compareAndSet(cmd,null);return false;}status("Snapp باز شد؛ وضعیت واقعی برنامه را بررسی می‌کنم…");SnappAccessibilityService.kickPending();return true;}
}
